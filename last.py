...
def creatematrix(row,column):
    ...
    if flag==0:
        ...
        print("")
    if flag==1:
        print('Введите каждую строку матрицы в формате "число пробел"')
        temporary_matrix=[]
        matrix=[]
        for i in range(0,row):
...
def conditions( n1, m1, n2, m2, matrix1, matrix2):
    ...
    if flag==3:
        print("Введенные матрицы непригодны для перемножения, НО вы можете их преобразовать, чтобы перемножить")
        print("введите 0 чтобы не преобразовывать, 1 чтобы транспонировать первую матрицу, 2 чтобы транспонировать вторую > ")
        flag3=accept()
        if flag3==1:
            matrix1=transpose(matrix1)
            print('\nТранспонированная Первая матрица:')
